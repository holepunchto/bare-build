...
module.exports = async function* createMSIXContentDirectory(
  ...
  const binary = PE.Binary.parse(await fs.readFile(prebuild))
  ...
  const appxManifestPath = path.join(app, 'AppxManifest.xml')
  ...
  const variables = {
    ...
    author,
    language,
    logo: `Assets\\Logo${path.extname(icon)}`,
  ...
  if (appxManifest) {
    await fs.writeFile(
      appxManifestPath,
      tpl.render(await fs.readFile(path.resolve(appxManifest)), variables)
    )
  } else {
    await fs.writeFile(appxManifestPath, createAppxManifest(variables))
  }

  yield appxManifestPath

  const appIcon = path.join(icons, 'Logo' + path.extname(icon))
