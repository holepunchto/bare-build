    ...
    if (sign) {
      ...
      if (entitlements) args.push('--entitlements', path.resolve(entitlements))
        ...
        if (provisioningProfile) {
          const info = path.join(out, 'Info.plist')

          await run('security', ['cms', '-D', '-i', path.resolve(provisioningProfile), '-o', info])

          await run('plutil', ['-extract', 'Entitlements', 'xml1', '-o', entitlements, info])
        } else {
