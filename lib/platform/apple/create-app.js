...
module.exports = async function* createApp(base, bundle, preflight, hosts, out, opts = {}) {
  ...
  const infoPlistPath = path.join(main, 'Info.plist')
  ...
  const variables = {
    ...
    minimumVersion,
    iconKey: assetCatalog ? 'CFBundleIconName' : 'CFBundleIconFile',
    iconValue: assetCatalog ? iconName : 'app'
  }
  ...
  if (infoPlist) {
    await fs.writeFile(
      infoPlistPath,
      tpl.render(await fs.readFile(path.resolve(infoPlist)), variables)
    )
  } else {
    await fs.writeFile(infoPlistPath, createPropertyList(variables))
  }
  ...
  if (assetCatalog) {
    const catalog = path.join(resources, 'Assets.car')
    await fs.copyFile(path.resolve(assetCatalog), catalog)
    await sign(catalog, opts)
    yield catalog
  } else {
...
function createMacOSPropertyList(opts) {
  const { name, identifier, version, author, language, minimumVersion, iconKey, iconValue } = opts
