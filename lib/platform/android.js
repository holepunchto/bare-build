...
module.exports = async function* android(base, bundle, preflight, opts = {}) {
  const { createAppBundle, createAPK } = require('bare-apk') // Optional
  ...
  try {
    ...
    await fs.writeFile(appManifest, createAndroidManifest(opts))
    yield appManifest
...
function createAndroidManifest(opts = {}) {
  const { name, identifier = toIdentifier(name), minimumSDK = 31, targetSDK = 36 } = opts
