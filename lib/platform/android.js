...
module.exports = async function* android(base, bundle, preflight, opts = {}) {
  const { hosts = [], standalone = false, package: pkg = false, out = '.' } = opts
  ...
  if (pkg) return [yield* createAppBundle(base, bundle, preflight, hosts, out, opts)]
  ...
  try {
    ...
    const aab = yield* createAppBundle(base, bundle, preflight, hosts, build, opts)
